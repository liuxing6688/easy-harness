#!/usr/bin/env node
/**
 * 幂等初始化 docs/ 目录骨架 + process.md，并同步 `.claude/harness-state.json`。
 *
 * 职责：为门禁提供「活跃 process.md 指针」。Hook 通过 harness-state.json
 * （或环境变量 `HARNESS_PROCESS_PATH`）决定读哪一份流程文件。
 *
 * 用法：
 *   node .claude/scripts/bootstrap-docs.mjs                  # Greenfield → docs/
 *   node .claude/scripts/bootstrap-docs.mjs --feature=<name> # Feature → docs/<name>/
 *
...
const PROCESS_TEMPLATE = path.join(PROJECT_ROOT, '.claude/templates/process.md');
const HARNESS_STATE = path.join(PROJECT_ROOT, '.claude/harness-state.json');
