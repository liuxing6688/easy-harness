...
test('TE 冒烟: isAlternativeE2eStartupCommand 识别 E2E_WEB_SERVER_COMMAND', () => {
  assert.equal(
    isAlternativeE2eStartupCommand(
      'E2E_WEB_SERVER_COMMAND=npx vite-node server.ts node .claude/scripts/e2e-run.mjs',
    ),
    true,
  );
  assert.equal(isAlternativeE2eStartupCommand('node .claude/scripts/e2e-run.mjs --scope=batch'), false);
...
test('TE 冒烟: 非 TE 派发时替代启动不拦截', () => {
  ...
  const r = checkTeAlternativeE2eStartup(
    'E2E_WEB_SERVER_COMMAND=npx vite-node x node .claude/scripts/e2e-run.mjs',
  );
