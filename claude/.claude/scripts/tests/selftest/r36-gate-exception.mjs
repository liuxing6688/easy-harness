...
test('R36: 写文件通道判定期异常 → deny（而非历史 allow）', () => {
  ...
  assert.match(output.user_message, /R36/);
  assert.match(output.user_message, /fail-closed/);
});
...
test('R36: Task 通道 → deny（否则「抛异常」即可跳过整条 R13 门禁链）', () => {
  ...
  assert.equal(verdict, 'deny');
  assert.match(output.user_message, /角色派发/);
});
...
test('R36: stop 通道 → followup（该通道无 deny 语义，收紧即不放行收尾）', () => {
  ...
  assert.equal(typeof output.followup_message, 'string');
  assert.equal(output.permission, undefined, 'stop 通道不得输出 permission 字段');
  assert.match(output.followup_message, /不放行/);
});

test('R36: 所有通道的文案都指向「用户本人改 config」这一唯一放松路径', () => {
  for (const channel of ['write', 'shell', 'task', 'toolchain', 'stop']) {
    const { output } = build(channel);
    const text = `${output.agent_message ?? ''}${output.followup_message ?? ''}`;
...
test('R36: 单独写活跃 process.md → 给修复例外（防「代理无法自愈」死局）', () => {
  ...
  assert.equal(repair('StrReplace', [ACTIVE_PROCESS]).length, 1);
});
...
test('R36: 异常摘要被截断，不把整段堆栈灌进裁决文案', () => {
  ...
  assert.ok(output.user_message.length < 600, '裁决文案未截断异常摘要');
});
