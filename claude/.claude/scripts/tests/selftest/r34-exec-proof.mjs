...
test('R34: 按运行器命令识别产物类别（e2e 按 --scope 分流）', () => {
  assert.equal(detectRunnerExecProofKind('node .claude/scripts/lint-run.mjs'), 'lint');
  assert.equal(detectRunnerExecProofKind('node .claude/scripts/static-scan-run.mjs'), 'static-scan');
  assert.equal(
    detectRunnerExecProofKind('node .claude/scripts/startup-smoke-run.mjs'),
    'startup-smoke',
  );
  assert.equal(
    detectRunnerExecProofKind('node .claude/scripts/e2e-run.mjs --scope=batch --required-ids=R-001'),
    'e2e-batch',
  );
  assert.equal(
    detectRunnerExecProofKind('node .claude/scripts/e2e-run.mjs --scope=final'),
    'e2e-final',
  );
...
test('R34: 非运行器命令不签发（不制造无人消费的私钥）', () => {
  ...
  assert.equal(detectRunnerExecProofKind('node .claude/scripts/qe-run.mjs'), null);
...
test('R34: 陈旧产物在 R15 判据上报执行证明类理由（而非「lint 有问题」）', () => {
  fixtureProcess(['---', 'workflow_mode: full', '---', ''].join('\n'), {
    'src/index.js': '// source file to establish source tree\n',
  });
