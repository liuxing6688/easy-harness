...
import {
  ...
  recordHotfixP0SoftReminder, recordFailOpenEvent, getTestProcessPath,
} from './_harness.mjs';
...
test('R9 软性提醒: recordHotfixP0SoftReminder 幂等——同一 process.md 不重复写入', () => {
  ...
  const first = fs.readFileSync(getTestProcessPath(), 'utf8');
  const r2 = recordHotfixP0SoftReminder(content);
  assert.equal(r2.ok, true);
  assert.equal(r2.reason, 'already-recorded');
  const second = fs.readFileSync(getTestProcessPath(), 'utf8');
