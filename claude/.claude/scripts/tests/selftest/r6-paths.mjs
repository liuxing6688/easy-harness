/**
 * R6：源码/扩展名路径门禁、.claude/scripts|agents|hooks 纳入、扩展名豁免目录。
 *
...
console.log('== R6：.claude/ 门禁路径判定 ==');
test('R6: .claude/scripts 下的文件受门禁保护', () => {
  assert.equal(isGatedDevPath('.claude/scripts/foo.mjs'), true);
});
test('R6: .claude/agents 下的文件受门禁保护', () => {
  assert.equal(isGatedDevPath('.claude/agents/foo.md'), true);
});
test('R6: .claude/hooks 下的文件受门禁保护（豁免标记文件除外）', () => {
  assert.equal(isGatedDevPath('.claude/hooks/gate-foo.mjs'), true);
  assert.equal(isGatedDevPath('.claude/hooks/.toolchain-install-approved.json'), false);
});
test('R6: 治理配置文件不走 DE 源码门禁（改由 R29 自治资产分级裁决）', () => {
  ...
  assert.equal(isGatedDevPath('.claude/harness.config.json'), false);
  assert.equal(isGatedDevPath('.claude/hooks.json'), false);
  assert.equal(isGatedDevPath('.claude/harness-state.json'), false);
});
