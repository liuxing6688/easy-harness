...
| R5 | 顶层代理不得代行子角色职责；机械化：`conversation_id` 顶层拦截 + 角色↔路径匹配（含 docs 成果物）+ 最近 Agent 派发落盘 | `CLAUDE.md` §5.1；`isRootConversationCaller` / `checkRolePathPermission` / `recordDispatchedRole`；`gate-subagent-track` / `gate-dev-workflow` |
| R6 | `.claude/scripts\|agents\|hooks/**` 三目录纳入机制门禁；代码扩展名默认受门禁 | `mechanical-gates.md` §8.1 Hook 一览表；§8.5「R6 加强」 |
| R8 | 禁止越级发起 Agent（**纯文字约束**，**F-27**：机械层校验的是门禁链**成果物存在性与有效性**（R13），**不校验 frontmatter `phase` 的阶段序关系**） | `CLAUDE.md` §5.8；能力边界见 `mechanical-gates.md` §8.1 |
...
| R10 | 流程终止不可逆：确认取消后 Hook 永久冻结 `process.md`（仅留 PM 逃生口） | `workflow-modes.md`；`gate-chain.md`「R10 的 `project-manager` 例外」；`CLAUDE.md` §5.19；`isCancelledProcessFile` / `isProcessFilePath` |
...
| R12 | 元规则：只可新增/加强门禁约束，不可放松；文档强于实现须补实现；实现严于声明到不可达亦属缺陷，但修正方向属放松，**须经用户确认并留痕，代理不得自行判断** | `CLAUDE.md` §2 强制规则第 5 条；理据与留痕范例见 `mechanical-gates.md` §8.5「门禁强度调整留痕」 |
...
| R20 | 轻量模式（hotfix / docs-only / single-task）须 AskUserQuestion +「工作流模式确认」机读行后才生效；未确认 fail-safe 为 `full` 并拒绝受门禁角色 Agent | `workflow-modes.md`「R20」（唯一权威定义，含固定选项文案）；`hasLiteModeConfirmation` / `checkLiteModeConfirmed` / `getWorkflowMode`；`checkRoleDispatchGate` |
...
| R28 | Shell 侧写文件门禁：写文件类命令按解析出的目标路径套用与 Write **同等**判据；目标不可解析的内联写入 deny、不可判定的工作树改写 ask；框架自带运行器豁免 | `mechanical-gates.md` §8.5（唯一权威定义）；`classifyShellWriteIntent`（`paths.mjs`）；`gate-dev-shell.mjs` |
| R29 | 门禁自治资产一律 **deny**（代理不得写入，含 Shell 通道）：运行时标记、授权凭证、台账、`hooks.json` / `harness.config.json` / `CLAUDE.md` / `harness/spec/**`；`harness-state.json` 归 PM、`gated-artifacts.json` 归 SA；放松型旋钮不再被合并 | `mechanical-gates.md` §8.5（唯一权威定义）；`classifyHarnessSelfGovernedPath` / `harnessSelfGovernedVerdict` / `hasToolchainInstallApproval`（`paths.mjs`） |
...
| R37 | `single-task` = 增量迭代档：**省**测试轮次折叠 + R26 豁免；**不省** R14/R17/R32/R15/R16/R18/R25/R19/R27/R33；前置为基线 `detail-design-spec.md` + 「## 增量范围」**五维**声明 + 破坏性变更（需迁移/破坏兼容）硬禁用 + **F-08** 兼容性回归对价（形状变而兼容未破时须声明并落地兼容性回归用例） | `workflow-modes.md`「`single-task` = 增量迭代档」（唯一权威定义）；`gate-chain.md`「`single-task` 模式门禁链」；`mechanical-gates.md` §8.2 R11 脚注 / §8.3 / §8.5 F-08 留痕 / §8.8；`checkSingleTaskPreconditions` / `checkIncrementScopeDeclared` / `checkSingleTaskBaseDesign` / `isCompatOnlySchemaChange`（`iteration.mjs`）；`checkIncrementCompatRegressionReport`（`qe.mjs`）；`parseWorkflowState`（`dispatch.mjs`） |
...
> 编号不连续（如无 R1/R4/R7）属正常：这些编号源自本框架自举开发（governance-overhaul）迭代中的需求/任务追溯标识，对应的 `requirement-list.md`/`develop-task-list.md` 是运行时产物，不随框架模板分发；本表只收录当前仍在 `CLAUDE.md`/Hook 正文中被引用、因而需要跨章节定位的编号。
