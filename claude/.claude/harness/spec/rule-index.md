...
| R29 | 门禁自治资产一律 **deny**（代理不得写入，含 Shell 通道）：运行时标记、授权凭证、台账、`hooks.json` / `harness.config.json` / `CLAUDE.md` / `harness/spec/**` / **`rules/**.md`**；`harness-state.json` 归 PM、`gated-artifacts.json` 归 SA；放松型旋钮不再被合并 | `mechanical-gates.md` §8.5（唯一权威定义）、§8.9（规则层）；`classifyHarnessSelfGovernedPath` / `harnessSelfGovernedVerdict` / `hasToolchainInstallApproval`（`paths.mjs`） |
