...
description: Conducts Harness Engineering project retrospectives against easy-harness spec (CLAUDE.md, hooks, agents, artifacts). Evaluates workflow compliance, proposes spec improvements for user approval, then implements and tests approved changes. Use only when the developer manually invokes project retrospective, post-mortem, or harness spec review.
...
>
> **规约权威源**：`CLAUDE.md`（常驻 charter §1–§8）、`.claude/harness/spec/`（说明权威细则）、`README.md`（自测与配置）、`.claude/hooks/**`、`.claude/agents/**`、`.claude/templates/**`。
>
> **元规则 R12**：规约改动**只可加强、不可放松**；放宽约束须拒绝并说明替代方案（如拆分迭代、补机械门禁而非删文档）。

## 前置：定位复盘范围

1. 确认工作区根目录存在 `CLAUDE.md` 与 `.claude/harness.config.json`（easy-harness 或已接入的宿主项目）。
2. 读取 `.claude/harness-state.json` 的 `activeProcessPath`；无则默认 `docs/process/process.md`。
3. 与用户确认复盘对象：
...
| 规约 | `CLAUDE.md`、`README.md`、`.claude/agents/*.md`、`.claude/hooks/**` |
...
- **顶层代理边界**：是否存在代写源码/设计、越级 Agent、Hook 绕过等（`CLAUDE.md` §5，结合 git 历史或对话记录若可获）。
...
| 改进方案 | 具体改哪些文件（`CLAUDE.md` / `.claude/harness/spec/` / agent / hook / 模板 / README） |
...
1. 先改**机械层**（`workflow-gate-lib.mjs`、各 `gate-*.mjs`、`e2e-run-lib.mjs`）再改**文档层**（`CLAUDE.md`、`.claude/harness/spec/`、`README.md`、agents、templates），保持 TG-D-4 / R13 表述一致。
...
```bash
npx vitest run --config .claude/scripts/vitest.config.ts
```
...
若 Phase 3 新增机械判定，在 `.claude/scripts/tests/selftest/` 或 `.claude/scripts/tests/scenarios/` 对应规则/场景文件中补最小回归用例（入口仍为 `gate-selftest.mjs` / `gate-scenarios.mjs`），并在变更清单中注明。
...
- **治理改动归属**：修改 `.claude/scripts|agents|hooks/**` 时，若当前项目仍有活跃开发流程，应提醒用户此类改动属 `governance-overhaul`；本 skill 在复盘场景下由开发者手动触发，可直接实施已批准项。
