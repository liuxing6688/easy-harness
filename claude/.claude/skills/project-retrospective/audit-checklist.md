...
| # | 检查项 | 规约依据 |
| --- | ------ | -------- |
| B9 | 无业务源码/构建产物/`e2e/**` 写入（文档与仓库元文件可改） | `workflow-modes.md` |
...
| B11 | `## 增量范围` **五维**齐全、「是否涉及」为是/否、「说明」有实质内容 | R37 |
| B12 | 增量范围声明与**实际改动**相符（抽查 git diff：声明「不涉及接口」却新增了端点属虚假声明） | R37 |
| B13 | 未涉及**需要迁移脚本 / 破坏向后兼容**的变更（抽查 git diff：若实际提交了迁移脚本、改了字段类型/主键/表结构或删改既有字段，却声明「否」，属**虚假声明**，严重违规） | R37、F-08 |
| B13a | 若声明「数据形状变更：是 + 需要迁移/破坏兼容：否」：①两维「说明」列已写明兼容性回归用例；②本轮唯一测试报告中有该用例的数据行且结果为通过；③抽查实现确为向后兼容（新增字段可选/有默认值、读取侧容忍缺失） | F-08、R37 |
