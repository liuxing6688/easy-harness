#!/usr/bin/env node
/**
 * PreToolUse 门禁（Bash / PowerShell）——薄适配器，判据全部复用 lib。
 *
 * 移植自 cursor/.cursor/hooks/gate-dev-shell.mjs：判定顺序与文案逐条照搬，
 * 仅翻译两处 I/O 契约（见下），**不重写任何判据**。
 *
 ...
 * 历史重写版完全丢失了这一职责（零处 exec-proof 调用），导致任何运行器产物都无法验签，
 * stop 门禁的 R34 分支因而永远拿不到「已签发」的产物——勿再回退。
 *
 ...
 *
 * ── Claude Code 契约翻译（唯一与 Cursor 版的差异）────────────────────────────
 * 1. 裁决出口：lib 的 `allow/deny/ask` 已统一输出官方 PreToolUse 形状
 *    （`hookSpecificOutput.permissionDecision`），故正常判据路径可直接复用；
 *    但 `buildGateExceptionVerdict` 返回的是 **legacy 形状**（`{permission,user_message,
 *    agent_message}`，被 gate-selftest 的 R36 断言钉死），故 R36 出口须在此翻译。
 * 2. 调用方身份：Cursor 用 `conversation_id`（每个子代理各一个）。Claude Code 的
 *    `session_id` 在子代理间**共享**，能区分的是 `agent_id`（仅子代理内出现）。
 *    故取 `agent_id ?? conversation_id ?? session_id`——子代理必有 agent_id 而与顶层基准
 *    不相等；顶层调用无 agent_id，回落到 session_id 恰好等于 subagentStart 落的基准。
 *
 * 自锁防护（§8.4 / **R36**）：**lib 加载失败** fail-open；**判定期异常**默认 fail-closed → deny。
 */

const HOOK_NAME = 'gate-dev-shell';

/**
 * 官方 PreToolUse 裁决出口。仅用于 lib 不可用 / R36 翻译等 lib 出口够不到的场景；
 * 正常判据路径直接用 lib 的 allow/deny/ask（同源形状）。
 ...
 * 那条路径已在 gate-dev-workflow-enhanced 里保留了例外，此处再开一个只会平添绕过面。
 *
 * `buildGateExceptionVerdict` 的 `output` 是 legacy 形状（R36 selftest 断言
 * `output.permission === 'deny'`，不可改 lib），故此处按官方契约翻译：
 *   permission → permissionDecision；user_message → permissionDecisionReason；
...
function failClosedDeny(context, err, lib) {
  ...
  emit(output.permission ?? 'deny', output.user_message, output.agent_message);
}

/**
 * 调用方身份（见文件头「契约翻译」第 2 条）。
 * @param {object} input
 * @returns {string|undefined}
 */
function resolveCallerId(input) {
  return input?.agent_id ?? input?.conversation_id ?? input?.session_id;
}
...
async function main() {
  ...
  try {
    ...
    const callerId = resolveCallerId(input);
      ...
      if (execProofKind) {
        try {
          ...
          process.stderr.write(
            `[${HOOK_NAME}] exec-proof issue failed: ${issueErr?.message ?? issueErr}\n`,
          );
        }
      }
      allow();
    ...
    // 运行器命令正是走这条早退路径（`node .claude/scripts/*-run.mjs` 不匹配包管理正则）。
