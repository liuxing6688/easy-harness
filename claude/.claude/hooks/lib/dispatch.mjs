  ...
  switch (role) {
    ...
    case 'test-engineer': {
      ...
      if (!lintClean.ok) {
        ...
        return { ok: false, reason: lintClean.reason, message: lintClean.message ?? 'R15：编程规范（lint）门禁未通过（.lint-result.json 缺失或 gatePassed≠true），QE 阶段须运行 `node .claude/scripts/lint-run.mjs` 并整改至通过；确无可用 linter 时须走「架构师声明 lintApplicability:"n/a" + 用户确认」双要素豁免。不得发起测试工程师。' };
      }
      const staticScanClean = checkStaticScanClean(content);
      if (!staticScanClean.ok) {
        return { ok: false, reason: staticScanClean.reason, message: staticScanClean.message ?? 'R16：静态代码质量门禁未通过（.static-scan-result.json 缺失或重复代码/安全扫描任一 gatePassed≠true），QE 阶段须运行 `node .claude/scripts/static-scan-run.mjs` 并整改至通过；确无法运行时须分别走「架构师声明 dupCheckApplicability/securityScanApplicability:"n/a" + 用户确认」双要素豁免。不得发起测试工程师。' };
      }
      return { ok: true, reason: 'checked' };
    }
    default:
