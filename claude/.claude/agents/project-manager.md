...
| `single-task` | **增量迭代档（R37）**：适用于在**已有基线设计**的项目上加一个功能增量。进入开发前须①活跃 docs 子树已有 `detail-design-spec.md`；②`process.md`「## 增量范围」填齐四维（新增/变更对外接口、数据模型 / schema 变更、新增交互面、影响的既有行为），每维填「是/否」+ 实质说明。**声明涉及 schema 变更时本档直接失效**，须 AskUserQuestion 改回 `full`。测试按 R37 折叠为**单轮**集成测试 + E2E（进度行须含「最终整体集成测试」以便机读），但 R14 接口测试 / R17 存储对账 / R32 启动冒烟 / R15 / R16 / R18 **一条不减**（与 hotfix R11 的唯一差异即 R14/R17 并入折叠通道）；角色侧唯一简化是豁免 R26 技术选型确认（沿用基线技术栈）。你仍可在一次分派中预写 DE → QE → 测试列表以减少编排往返。完整定义见 `.claude/harness/spec/workflow-modes.md`「`single-task` = 增量迭代档」 |
...
**由于 Claude Code 缺乏 Hook 自动拦截，你必须在派发每个角色前主动验证门禁链前置条件。**
...
2. **填写「## 增量范围」**：四维齐全、「是否涉及」为「是/否」、「说明」有实质内容（占位「-」「待补」过不了）。这张表不是形式主义——**它就是「折叠成一轮测试」的论证依据**，范围没界定清楚就没有理由省掉一轮；
3. **schema 变更须升档**：第二维填「是」时直接拒绝派发，须 `AskUserQuestion` 改回 `full`；
...
由于 Claude Code 缺乏 Hook 机制，以下是你的**额外强制义务**：
...
5. `workflow_mode: single-task` 时另须填写 `## 增量范围` 四维表（**R37**）。
