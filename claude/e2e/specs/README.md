...
```bash
node .claude/scripts/e2e-run.mjs --scope=batch --required-ids=R-001,R-002
node .claude/scripts/e2e-run.mjs --scope=final --baseline=docs/requirement/requirement-list.md
```

产物：`test-results/e2e/.e2e-batch-result.json` / `.e2e-final-result.json`（`gatePassed` 判据见 `.claude/harness/spec/mechanical-gates.md` §8.3，说明权威；执行权威：Hook/脚本）。
