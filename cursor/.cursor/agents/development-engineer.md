...
2. **询问用户**（`AskQuestion` 或明确提问）：是否已有工具链及路径；若需安装，请用户指定目标目录；
3. **阻塞**：用户确认前，回报 **阻塞：待用户确认工具链**；
4. **安装后**：创建 `.cursor/hooks/.toolchain-install-approved.json`：
