...
test('R6 加强: docs/ 与 .cursor/ 分支优先级不被扩展名规则改变（回归）', () => {
...
test('R29: 工具链授权凭证归类为 approval-marker（deny）', () => {
  const p = '.cursor/hooks/.toolchain-install-approved.json';
...
test('R29: Hook 注册表与门禁配置归类为 gate-config（deny）', () => {
  for (const p of ['.cursor/hooks.json', '.cursor/harness.config.json']) {
...
test('R29: 宪章与说明权威归类为 gate-config（改动等于调门禁口径）', () => {
  ...
  assert.equal(
    classifyHarnessSelfGovernedPath('.cursor/harness/spec/mechanical-gates.md'),
    'gate-config',
  );
});
...
test('R29: harness-state.json 纳入角色门禁，期望 project-manager', () => {
  assert.equal(isHarnessStatePath('.cursor/harness-state.json'), true);
  assert.equal(isGatedRoleArtifactPath('.cursor/harness-state.json'), true);
  assert.deepEqual(expectedRolesForPath('.cursor/harness-state.json'), ['project-manager']);
});
...
test('R28: 自治资产目标经 Shell 写入时按 R29 分级（而非普通路径判据）', () => {
  const forge = classifyShellWriteIntent(
    'Set-Content -Path .cursor/hooks/.dispatched-roles.json -Value "{}"',
  );
  assert.equal(forge.selfGoverned.some((s) => s.kind === 'runtime-marker'), true);
  const cfg = classifyShellWriteIntent('echo {} > .cursor/harness.config.json');
...
test('R28: 只读 / 构建 / 框架运行器命令不被判为写文件（防误伤）', () => {
  for (const cmd of [
    ...
    'node .cursor/scripts/e2e-run.mjs --scope=batch --required-ids=R-001',
