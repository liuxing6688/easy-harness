...
>
> **规约权威源**：`AGENTS.md`（常驻 charter §1–§8）、`.cursor/harness/spec/`（说明权威细则）、`README.md`（自测与配置）、`.cursor/hooks/**`、`.cursor/agents/**`、`.cursor/templates/**`。
>
> **元规则 R12**：规约改动**只可加强、不可放松**；放宽约束须拒绝并说明替代方案（如拆分迭代、补机械门禁而非删文档）。

## 前置：定位复盘范围

1. 确认工作区根目录存在 `AGENTS.md` 与 `.cursor/harness.config.json`（easy-harness 或已接入的宿主项目）。
2. 读取 `.cursor/harness-state.json` 的 `activeProcessPath`；无则默认 `docs/process/process.md`。
3. 与用户确认复盘对象：
...
| 规约 | `AGENTS.md`、`README.md`、`.cursor/agents/*.md`、`.cursor/hooks/**` |
...
| 改进方案 | 具体改哪些文件（`AGENTS.md` / `.cursor/harness/spec/` / agent / hook / 模板 / README） |
...
1. 先改**机械层**（`workflow-gate-lib.mjs`、各 `gate-*.mjs`、`e2e-run-lib.mjs`）再改**文档层**（`AGENTS.md`、`.cursor/harness/spec/`、`README.md`、agents、templates），保持 TG-D-4 / R13 表述一致。
...
- **治理改动归属**：修改 `.cursor/scripts|agents|hooks/**` 时，若当前项目仍有活跃开发流程，应提醒用户此类改动属 `governance-overhaul`；本 skill 在复盘场景下由开发者手动触发，可直接实施已批准项。
