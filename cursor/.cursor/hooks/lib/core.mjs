...
// R6：.cursor/scripts|agents|hooks 三目录纳入机制门禁；其余 .cursor/** 默认放行，
// 但可被 dotCursorExemptPatterns 精确豁免其中的非治理产物（如 .toolchain-install-approved.json）。
const DEFAULT_DOTCURSOR_EXEMPT_PATTERNS = [
  '.cursor/templates/**',
  '.cursor/rules/**',
  '.cursor/harness-state.json',
  '.cursor/hooks.json',
...
const DEFAULT_CONFIG = {
  gatedPaths: {
    ...
    dotCursorExemptPatterns: DEFAULT_DOTCURSOR_EXEMPT_PATTERNS,
  },
  gatedShellPatterns: [
    '\\bnpm\\s+create\\b',
    '\\bnpm\\s+install\\b',
...
export function getMergedDotCursorExemptPatterns() {
  const config = loadHarnessConfig();
  return config.gatedPaths.dotCursorExemptPatterns ?? DEFAULT_DOTCURSOR_EXEMPT_PATTERNS;
}
