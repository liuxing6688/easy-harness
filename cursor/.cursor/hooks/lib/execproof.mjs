 ...
 *
 * 消费方：`gate-dev-shell`（签发）、`.cursor/scripts/*-run.mjs`（落签）、
 ...
 *    **私钥**写入交接文件 `.exec-proof-pending/<nonce>.json`。两者都在 `.cursor/hooks/`
...
export function detectRunnerExecProofKind(command) {
  ...
  const m = command.match(
    /\.cursor[/\\]scripts[/\\](lint-run|static-scan-run|e2e-run|startup-smoke-run)\.mjs\b/i,
  );
  if (!m) return null;
 ...
 * 即 R15/R16/E2E/R32 这几项门禁真正在检查的东西。刻意**不含** `.cursor/**`：
