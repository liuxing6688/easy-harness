    ...
    const execProofFollowup = () =>
      exitFollowup(
        `【流程门禁】（R34 执行证明）以下门禁的机读产物未通过执行证明验签：${state.execProofFailedGates.join('、')}。` +
          ...
          '让 `beforeShellExecution` 门禁签发 nonce 并由运行器落签；**禁止**手工编辑 test-results 下的机读产物。' +
          '若本机确实无法经门禁通道运行（如用户自行在外部终端执行），须由**用户本人**在 `.cursor/harness.config.json` 设 `execProof.enforce: false`（R29 锁定，代理不得修改）。',
      );
    ...
    if (!isDocsOnly && state.qeComplete) {
      ...
      if (isHotfix) {
        ...
        if (!state.finalE2ePassed) {
          exitFollowup(
            '【流程门禁】（R37 single-task 折叠通道）测试记录已完成，但 E2E 门禁未通过。请由 test-engineer 运行 `node .cursor/scripts/e2e-run.mjs --scope=final --baseline=<requirement-list.md 或本次增量范围>`；`gatePassed` 为 true 前禁止宣告完成。',
          );
        }
