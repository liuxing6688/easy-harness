...
> 本文件由 project-manager 维护；顶层代理只读并据此发起 Task，不得自行改写以跳过门禁（`AGENTS.md` §5）。
...
> 测试收尾已知缺陷示例：`| 测试收尾已知缺陷 | YYYY-MM-DD | AskQuestion 确认接受带已知缺陷交付；未关闭高严重/须 DE 修复项：… |`（测试报告有未关闭高严重或「须 DE 修复」时，PM 收尾前须 AskQuestion 留痕，禁止静默非阻塞收尾）。
...
> Hook fail-open（`.cursor/harness/spec/mechanical-gates.md` §8.4）时由门禁脚本自动追加；项目经理处理后将「处理状态」改为「已处理」并清除 `blocking`。出厂为空表，不构成阻塞。
