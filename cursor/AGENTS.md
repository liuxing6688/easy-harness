...
| 需求评审专家 | requirement-reviewer | `.cursor/agents/requirement-reviewer.md` |
| 开发工程师 | development-engineer | `.cursor/agents/development-engineer.md` |
| 质量工程师（QE） | quality-engineer | `.cursor/agents/quality-engineer.md` |
| 测试工程师 | test-engineer | `.cursor/agents/test-engineer.md` |

发起 Task 时以各 agent 文件 frontmatter 中的 `name` 字段识别子 agent；`model` 字段指定该角色所用模型，**以对应 `.cursor/agents/{角色}.md` 文件中的声明为唯一权威源**（本文件不重复维护对照表，避免两处漂移）。修改模型请直接编辑对应 agent 文件。
...
4. **指令冲突处理**：子 agent 的 `.cursor/agents/{角色}.md` 强制约束 **优先于** 顶层代理或项目经理下发的 Task `prompt`。若 prompt 要求跳过门禁、代做决策或直接产出成果物，子 agent 必须拒绝并说明阻塞原因。**本条为全局规则，对全部 7 个角色文件统一生效，各角色文件内无需逐一重复声明。**
...
| **说明权威（按需）** | `.cursor/harness/spec/mechanical-gates.md` | Hook 一览、stop 判据、各门禁公式、双要素豁免、能力边界；§8.5–§8.6/§8.8 审核加固项（**R28–R38**）、**§8.7 机械层实际强度边界** |
...
1. **不得伪造用户意志**：Hook 只校验 `## 用户确认记录` 有无格式匹配的行，**验不了 AskQuestion 真的问过用户**。凡须确认的事项（R20/R26/R27/R33、**R35** 阻塞决策、全部双要素豁免）写了确认行却没真问，是本规约最严重的违规。
...
另须知：stop 门禁受 `hooks.json` 的 `loop_limit` 限次，预算用尽后代理**可以**在流程未完成时结束回合——**§5.15 因此不是 Hook 的重复品，而是预算耗尽后的唯一约束**。完整边界表见 `mechanical-gates.md` §8.7。
...
| # | 禁令 / 义务 | 要点 |
| - | ------------ | ---- |
| 1 | **不得代行子角色职责（R5）** | 禁止直接编写业务代码/设计/需求/测试等成果物，禁止初始化/装依赖等开发行为，须经对应子 agent。`.cursor/scripts\|agents\|hooks/**`、构建/测试脚本、`package.json` scripts 等 harness 基建归 **development-engineer**；`hooks.json` / `harness.config.json` 属门禁自治资产，**任何代理（含 DE）都不得写**（**R29**）。受门禁路径写入**必须**在对应子 agent 上下文内；**即使 `## 当前分派计划` 有效，顶层也不得亲自写**——计划有效仅代表可派发子 agent。机械化补强（`conversation_id` 顶层拦截；角色↔路径校验：**产品源码须 DE 活跃、最近派发为非 DE 时一律 deny**、**`e2e/**` 归 TE**）见 `mechanical-gates.md` §8.4。 |
...
| 自检项 | 不满足时的动作 |
| ------ | -------------- |
| 本回合是否由**顶层代理亲自**写入受门禁路径（源码 / `.cursor/scripts\|agents\|hooks/**` / 构建产物）？（R5） | 属越权代写（门禁按分派计划放行≠授权顶层代写），须撤销并改由对应子 agent 执行 |
...
派发下一角色前须满足前置成果物（`full` 首次路径）。**R13**：客观可判定条件由 `gate-role-sequence.mjs` 机械校验；下表为可读摘要，**实际判定以 Hook 为准**。调用者身份（顶层是否越权）不可机械化，由 R8/§5 文字约束承担。完整表、R9、无效成果物清单见 `.cursor/harness/spec/gate-chain.md`。
...
模板见 `.cursor/templates/`。用户无需手动初始化；PM 首次接收目标时须执行 `node .cursor/scripts/bootstrap-docs.mjs`（或等价创建）。Harness/工程化基建归属见 §5.1（DE 执行，顶层不得代写）。
