...
> **路径解析**：Hook 注册命令相对于 Codex session `cwd` 调用适配器，不调用 Git 或查找 Git root。适配器再从 Hook payload 的 `cwd` 设置 `HARNESS_PROJECT_ROOT`，门禁代码通过 `import.meta.url` 定位自身脚本；因此门禁业务根与 Hook 文件所在层级解耦。直接运行 legacy gate 时，`HARNESS_PROJECT_ROOT` 缺省回退到适配目录的 workspace 根。
...
Hook 解析 `## 进度列表` 时同时识别中文角色名与 `.codex/agents` 的 agent slug（如 `开发工程师` / `development-engineer`），项目经理可按 Task 实际发起名称留痕。
...
| **工具不可用**（**R38**，非 docs-only） | `qeComplete && toolUnavailableGates.length > 0` | 失败源于检查工具本身不可用（依赖拉取/网络/代理/证书/命令缺失），**不是**代码质量问题。要求 PM 标 `blocking` + AskQuestion 请用户在「修工具 / 配等价命令覆盖 / 走双要素豁免」间决策；**不得**按「整改质量问题」处理（唯一权威定义见 §8.8） |
...
> - 判据结构与 E2E 门禁同构（运行器写 `gatePassed` 机读产物 → lib 读入 → 门禁判定）；**执行命令与产物**：`node .codex/scripts/lint-run.mjs` → `test-results/qe/.lint-result.json`。
...
> - 判据结构与 R15 同构，但**跨技术栈通用、不做 per-stack 探测**（本框架要求 `Node.js >= 18`，两项工具均经 `npx` 直接获取）；**执行命令与产物**：`node .codex/scripts/static-scan-run.mjs` → `test-results/qe/.static-scan-result.json`（含 `duplication`/`security` 两个子结果）。
...
> 本节是「批次/最终 E2E」判据与命令的**唯一权威定义**；`README.md`、§3、§5、§6、`project-manager.toml`、`test-engineer.toml` 中出现的相关表述均须与本节保持一致，若只需引用判据请指回本节，不再复述完整公式/命令。

- **两级范围**：①**批次集成测试**——每批次 QE 通过后，对本批次新交付任务包做集成测试；②**最终整体集成测试**——全部任务包与各批次 E2E 闭环后，对整个产品做端到端集成测试。`测试判定`（最终交付依据）以**最终整体集成测试**（含最终 E2E）结论为准。
- **执行命令与产物**：批次 `node .codex/scripts/e2e-run.mjs --scope=batch --required-ids=<本批次P0>` → `test-results/e2e/.e2e-batch-result.json`；最终 `node .codex/scripts/e2e-run.mjs --scope=final --baseline=<requirement-list.md>` → `test-results/e2e/.e2e-final-result.json`。
...
- **接口测试适用性豁免（无对外接口项目）**：纯算法库、纯静态前端、无 HTTP/RPC/CLI 契约的组件等**无对外接口**项目，可豁免 R14 接口测试判据；判定遵循 §8.2「双要素豁免机制」表 R14 行（两项皆满足时 `isApiTestExempt()` 使 `batchApiReportPresent` 视为满足）。详见 `test-engineer.toml`「接口测试适用性豁免」。
...
| **判定期异常**（`runtime`） | **fail-closed**（**R36**，默认）：write/shell/task → `deny`，toolchain → legacy `ask`（Codex 适配后为 `deny`），stop → `followup` | 见下方 R36 说明 |
...
> **Codex 适配说明**：项目 `hooks.json` 不使用 Cursor 的 `failClosed` 字段。判定期异常由 legacy 入口脚本输出明确裁决，再由 `codex-hook-adapter.mjs` 转为 Codex JSON；适配器自身或子进程异常则以非零退出结束。收紧来自脚本与适配器的 fail-closed 行为，不依赖不存在的项目配置字段。
>
...
- Hook 对**源码 / 构建产物 / 根目录敏感产物 / `.codex/scripts|agents|hooks/**` 三目录 / `docs` 下角色成果物（需求/设计/质量/测试/`process.md`）/ 受门禁 Shell 命令 / Task 发起前的角色前置成果物（R13）**做确定性拦截。`.codex/hooks.json`、`.codex/harness.config.json` 不走 DE 源码门禁，但**已纳入 R29 门禁自治资产分级**（写入须人工批准，见 §8.5），不再仅由文字约束覆盖。
- **R5 调用者身份（Codex 等效实现）**：Codex PreToolUse 不提供稳定的当前 subagent id，且 SubagentStart 的 `session_id` 是父会话 id，故不使用 `conversation_id` 区分调用者。项目配置将顶层设为 `read-only`，角色 TOML 设为 `workspace-write`；写入期仍由 `checkRolePathPermission` 校验期望角色与分派/进度/最近派发。产品源码须 DE 活跃，`e2e/**` 期望 `test-engineer`；R22/R24 与 R32 的其余判据保持不变。CLI 参数或用户配置可覆盖项目 sandbox，故 `--yolo` 等仍会削弱该隔离，见 §8.7。
...
`.codex/scripts/tests/selftest/r28-r31-hardening.mjs`，Hook 端到端见
`.codex/scripts/tests/scenarios/hardening.mjs`。
...
`dotCodexExemptPatterns` 中，等于「被约束方可自由改写约束自身」。实测一步可达：
...
**为什么一律 `deny` 而不是 `ask`（重要，勿回改）**：Codex 官方 Hooks 文档明确说明
`PreToolUse` 的 `permissionDecision:"ask"` 当前不支持，返回会被标记失败并继续工具调用。
因此适配器把旧内核 `ask` 保守转为 `deny`；真正的用户批准由 `PermissionRequest` / 原生 sandbox
流程承担。R29 在 Shell 通道仍刻意 deny，防止换工具绕过。
...
**残留缺口（坦诚披露）**：`.codex/templates/**` 仍为豁免——
模板不被 Hook 直接读取（门禁读 `docs/` 成果物），但被污染的模板
...
**Codex 适配说明**：该逻辑只为保留旧内核自测与健康记录，不再承担顶层隔离。
顶层隔离由根 read-only sandbox 承担；角色路径判据继续机械执行。
...
`target`、`.venv`、`test-results` 等）。`.codex/`、`docs/`、`e2e/` 分支优先级不变。
...
**所有夹具都是套件内自拼的 Markdown 字符串，从不加载 `.codex/templates/` 下的真实文件**——
...
| Maven / Gradle / PHP / .NET 默认命令为空，而 `harness.config.json` 受 **R29** deny | 唯一补救手段对**所有代理**关闭，流程必然停在 QE；`system-architect.toml` §5 第 3 条却仍指示架构师去写该文件——**规约给出的处方，执行层会直接拒绝** | ①为工具链自带分析器的栈补默认（见下方留痕）；②Maven/Gradle/PHP/CMake/Make 仍留空但产出 `remediation`（候选命令 + 可粘贴片段）；③改写 `system-architect.toml` §5、`detail-design-spec.md` §5 与 `quality-engineer.toml` R15 节：架构师**呈现片段、请用户本人粘贴**，不再指示其写 config | `r15-lint.mjs` 新栈默认值与 `remediation` 用例 |
...
回归用例：单元级见 `.codex/scripts/tests/selftest/r32-startup-smoke.mjs` 与
`r33-ui-expectation.mjs`，Hook 端到端见 `.codex/scripts/tests/scenarios/startup-smoke.mjs`
（SS0–SS7），纯函数另见 `.codex/scripts/startup-smoke-lib.test.ts`。
...
| 项 | 内容 |
| -- | ---- |
| **执行命令与产物** | `node .codex/scripts/startup-smoke-run.mjs` → `test-results/e2e/.startup-smoke-result.json` |
...
| **失败后果** | 属**产品缺陷**：TE 判定测试不通过 + `blocking: true` + 建议 PM 回派 DE（`rollback.md` 回退触发条件）。PM 的「接受带已知缺陷交付」AskQuestion **不能**解除本机读门禁 |
...
| 项 | 内容 |
| -- | ---- |
| **罗盘第 7 维** | `requirements-analyst.toml` §1.3 新增「交互与界面期望」；§1.3.1 规定必聊条件：greenfield / 引入新交互面 / 目标点名竞品或参照物 / 用户表达过任何外观偏好 |
...
| **下游消费** | SA 将 §3.4 作为设计输入（`system-architect.toml` 输入第 1 条）；RR「体验」维以 §3.4 为审核基线（`requirement-reviewer.toml`） |
...
> （阻塞释放阀比 Stop continuation 更便宜）已分别由 **R34**、**R35** 收紧，**但都没有消失**——
...
| - | ---- | ------------ | -------------- | ---- |
| 1 | **用户确认是自述** | 校验 `## 用户确认记录` 中确认行的**结构性存在**与关键词匹配（`hasLiteModeConfirmation` / `hasUiExpectationConfirmation` / 各 `is*Exempt`） | **无法验证 AskQuestion 真的问过用户**。确认行由 PM（被约束方）书写，双要素豁免的两个要素（`gated-artifacts.json` 与 `process.md`）**同样都由 AI 写** | 角色文件的 AskQuestion 义务（R20/R26/R27/R33）；复盘 skill 抽查确认行与实际对话是否对得上 |
...
| 3 | **Stop continuation 有递归保护** | 流程未闭环时把内核 `followup_message` 转为 Codex `decision:block` | Codex 没有 Cursor 的 `loop_limit`；适配器在 `stop_hook_active=true` 时返回 `continue:false`，只自动续跑一次，之后代理仍可在未闭环时结束回合 | `AGENTS.md` §5.15 回合结束前自检；不得以「Hook 会拦」为由省略 |
...
证据产物不设防、阻塞释放阀过于便宜、判定期异常曾默认放行、缺可用的轻量档、
工具不可用与代码不合格不可区分。回归用例：单元级见 `.codex/scripts/tests/selftest/r34-exec-proof.mjs`、
`r35-blocking-evidence.mjs`、`r36-gate-exception.mjs`、`r37-single-task.mjs`、`r38-tool-unavailable.mjs`；
Hook 端到端见 `.codex/scripts/tests/scenarios/audit-fixes.mjs`（AF1–AF14）。
...
| 步骤 | 执行者 | 内容 |
| ---- | ------ | ---- |
| **签发** | `gate-dev-shell`（`PreToolUse:Bash`） | 识别本次命令为框架运行器时生成 ed25519 密钥对：**公钥**写入台账 `.harness/.exec-proof-ledger.json`，**私钥**写入交接文件 `.harness/.exec-proof-pending/<nonce>.json` |
...
- **基准**：`gatedPaths.sourceDirs`（含 `gated-artifacts.json` 的收紧项）加 `e2e/` 下全部文件的最大
  mtime；刻意**不含** `.codex/**`——改 Hook 不应让全部质量产物失效。遍历跳过依赖/构建/受控运行
...
**R29 联动**：台账与私钥交接目录（前缀匹配 `.harness/.exec-proof-pending`）纳入
...
§8.7 曾把 stop 门禁的强度上限归结为自动 continuation 次数，但实际释放成本只是**一行 frontmatter**，
...
- **旁路台账** `.harness/.gate-exception-ledger.json`：由 `recordFailOpenEvent` 在 `process.md`
...
**问题**：入口脚本曾自行 `try/catch` 并以退出码 0 输出 legacy allow JSON，判定期异常的语义完全由脚本的 fail-open 决定，等于
...
| `toolchain` | `gate-toolchain-install` | legacy `ask`，Codex 适配后为 `deny` | `PreToolUse` 不提供可靠 ask；适配器拒绝本次调用并要求先在对话中取得用户决定，再走用户执行或原生批准路径 |
...
- **唯一角色侧简化**：发起 `requirement-reviewer` / `development-engineer` 时豁免 **R26** 技术选型确认
  （基线项目已 AskQuestion 确认并落痕，增量不换栈）。**R25 同构模块识别不豁免**——增量最容易
...
它改变的是**失败的性质与解法**：stop 门禁注入专门的 followup，要求 PM 标 `blocking` 并用 AskQuestion
