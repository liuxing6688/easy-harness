...
| R28 | Shell 侧写文件门禁：写文件类命令按解析出的目标路径套用与 Write **同等**判据；目标不可解析的内联写入 deny、不可判定的工作树改写在 legacy 内核为 ask、经 Codex 适配后为 deny；框架自带运行器豁免 | `mechanical-gates.md` §8.5（唯一权威定义）；`classifyShellWriteIntent`（`paths.mjs`）；`gate-dev-shell.mjs` |
