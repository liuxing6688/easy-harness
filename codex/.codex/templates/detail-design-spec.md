...
> **机制说明（R25，机读，2026-07-28 QE R16 消重复盘新增）**：本节须排查是否存在 ≥2 个结构相似的资源/页面/测试 fixture/E2E helper（如同构 CRUD 路由、页面脚手架、测试 fixture、E2E helper），并为每组声明共享 primitive（工具函数/基类/hook/schema）名称与目录落点，供 §3 目录结构与 `develop-task-list.md` 前置任务包呼应；避免并行开发工程师各自「复制改」导致 QE 阶段 R16 重复代码打回。**若确认无同构资源族**，须写「已排查，无同构资源族」并附排查依据（涉及范围/排查方式），不得留空。

| 同构组名称 | 涉及范围（资源/页面/测试 fixture/E2E helper） | 共享 Primitive 名称 | 落点路径 |
| ---------- | --------------------------------------------- | -------------------- | -------- |
| | | | |
...
> 「是否受门禁保护」须与 `gated-artifacts.json`（见 §7）及 `.codex/harness.config.json` 默认列表核对一致。

## 4. 接口 / 数据模型设计

（按项目技术栈：API 契约、数据库 schema、状态管理等）

**业务数据存储介质（R17 输入，须明确）**：声明本项目业务数据是否写入持久化/缓存介质；若写入，列出类别（与 `.codex/harness/spec/mechanical-gates.md` §8.3 R17 存储介质范围一致：数据库 / 文件 / 缓存 / 对象存储 / 其他）。无上述写入时可走 R17 双要素豁免。写路径涉及几种介质，测试对账就覆盖几种。
...
> **机制说明（R32，2026-07-29 启动报错复盘新增）**：测试工程师须对**本节声明的生产启动命令**跑两段冒烟（`node .codex/scripts/startup-smoke-run.mjs`）：①干净启动；②**强杀后再启动**。因此本节须给出可被机械执行的启动命令，并说明单实例/锁等互斥机制在异常退出后的恢复策略——复盘中 `npm run start` 因 ESM 导入失败、以及修好后再启动出现 `DATA_DIRECTORY_LOCKED`，均源于本节缺位。
>
...
> **机制说明**：QE 阶段运行 `node .codex/scripts/lint-run.mjs` 时，命令由 **`harness.config.json` → `qe.commands.lint` 覆盖 > 构建清单自动探测 > 下表默认值** 解析（与 Hook 机械判据一致）。本节由系统架构师按已确认技术栈**填入一行**，供 QE/PM 查阅；**不必**为通过门禁而重复写入 config——多数栈留空 `qe.commands.lint` 即可。
>
...
>
> **反弱化条款**：**禁止**以「减少打回」为由覆盖本命令调高阈值或扩大排除目录，详见 `.codex/harness/spec/mechanical-gates.md` §8.2 R16「反弱化条款」——确需覆盖须写明具体排除路径 + 理由，修改阈值须用户确认留痕。
...
| 集成测试 | test-engineer | | |
| 接口测试 | test-engineer | | 开发窗口批次集成测试阶段必测（R14，见 `.codex/harness/spec/mechanical-gates.md` §8.3）；覆盖各接口/契约 |
| 存储对账 | test-engineer | | 开发窗口批次机读硬门禁（R17，见 `.codex/harness/spec/mechanical-gates.md` §8.3 / `checkBatchStorageReconciliationReport`）；介质范围见 §4 |
| E2E（若适用） | test-engineer | Playwright Chromium headless | P0 场景，见 `.codex/harness/spec/mechanical-gates.md` §8.3；写路径对账留痕见 R17 |
| 生产启动冒烟 | test-engineer | `node .codex/scripts/startup-smoke-run.mjs` | 批次与最终两级机读硬门禁（R32，见 `.codex/harness/spec/mechanical-gates.md` §8.6）；启动命令与恢复策略见 §4「生产启动与异常恢复」 |

> 若项目无 UI 或不适用浏览器 E2E，须在 §7 `gated-artifacts.json` 中声明 `e2eApplicability: "n/a"` 并注明理由，等待用户在 `process.md`「用户确认记录」中确认豁免。
> 若项目无业务数据持久化，须声明 `storageReconciliationApplicability: "n/a"` 并经用户确认后豁免 R17（见 `.codex/harness/spec/mechanical-gates.md` §8.2）。
> 若项目确无可冒烟的常驻启动路径（纯库、纯静态资源包），须声明 `startupSmokeApplicability: "n/a"` 并经用户确认后豁免 R32；**「暂时起不来」不是豁免理由**——冒烟失败属产品缺陷，须回派开发工程师。

## 7. 受门禁保护的产物声明

已同步写入 `docs/[{feature}/]design/gated-artifacts.json`（模板：`.codex/templates/gated-artifacts.json`）。
...
- 依赖安全审计命令见 `quality-engineer.toml`「依赖审计命令」，执行结果记录于质量报告。
