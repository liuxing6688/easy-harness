...
const TOOLCHAIN_MARKER = path.join(PROJECT_ROOT, '.codex/toolchain-install-approved.json');
...
export function hardeningScenarios() {
  ...
  // R29：门禁自治资产
  // -------------------------------------------------------------------------
  check('SG1 R29：写 Hook 注册表 .codex/hooks.json 被拒（须用户本人改）', 'deny', {
    hook: 'write', filePath: '.codex/hooks.json', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG2 R29：写门禁配置 .codex/harness.config.json 被拒（须用户本人改）', 'deny', {
    hook: 'write', filePath: '.codex/harness.config.json', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG3 R29：伪造 R5 派发凭证 .dispatched-roles.json 被拒', 'deny', {
    hook: 'write', filePath: '.harness/.dispatched-roles.json', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG4 R29：改写 R5 身份基准 .root-conversation-id.json 被拒', 'deny', {
    hook: 'write', filePath: '.harness/.root-conversation-id.json', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG5 R29：自签工具链授权凭证被拒（须用户本人改）', 'deny', {
    hook: 'write', filePath: '.codex/toolchain-install-approved.json', processPath: readyProc, gatedPath: readyGated,
  });
  ...
  const shellCases = [
    ...
    ['SH8 框架运行器放行', 'node .codex/scripts/lint-run.mjs', 'allow'],
  ...
  check('SH12 Shell 伪造 R5 派发凭证被拒', 'deny', {
    hook: 'shell',
    command: 'Set-Content -Path .harness/.dispatched-roles.json -Value "{}"',
    processPath: noDispProc,
    gatedPath: noDispGated,
  });
