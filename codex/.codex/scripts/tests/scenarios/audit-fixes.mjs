...
export function auditFixesScenarios() {
  ...
  check('AF4 R34/R29：写执行证明台账 .exec-proof-ledger.json 被拒', 'deny', {
    hook: 'write',
    filePath: '.harness/.exec-proof-ledger.json',
    processPath: r34Proc,
  });
  check('AF5 R34/R29：写私钥交接文件被拒（自签执行证明）', 'deny', {
    hook: 'write',
    filePath: '.harness/.exec-proof-pending/abc123.json',
    processPath: r34Proc,
  });
  check('AF6 R34/R29：Shell 删除私钥交接目录被拒（抹掉「未消费」作废条件）', 'deny', {
    hook: 'shell',
    command: 'rm -rf .harness/.exec-proof-pending',
    processPath: r34Proc,
  });
  check('AF7 R34：运行器命令本身仍放行（签发通道不得反过来拦住运行器）', 'allow', {
    hook: 'shell',
    command: 'node .codex/scripts/startup-smoke-run.mjs',
    processPath: r34Proc,
  });
