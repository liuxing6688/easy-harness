 ...
 *   node .codex/scripts/gate-scenarios.mjs --verbose
 */
...
export const HOOKS_DIR = path.join(PROJECT_ROOT, '.codex/hooks');
...
export function buildPayload(hook, { role, filePath, command, conversationId, content }) {
  ...
  // R5 机械化补强测试用：模拟 Codex 真实 payload 里的 conversation_id 字段
...
export function writeLintNoCommand() {
  writeLintResultFixture({
    ...
    remediation: { configPath: '.codex/harness.config.json', suggestedCommand: null },
    executedAt: new Date().toISOString(),
  });
}
...
// R5 机械化补强：顶层会话 id 落盘于 .harness/.root-conversation-id.json——
// 与 lint/静态扫描产物同为受控运行产物，快照/还原避免污染宿主运行时。
const ROOT_CONVERSATION_FILE = path.join(RUNTIME_DIR, '.root-conversation-id.json');
...
const DISPATCHED_ROLES_FILE = path.join(RUNTIME_DIR, '.dispatched-roles.json');
