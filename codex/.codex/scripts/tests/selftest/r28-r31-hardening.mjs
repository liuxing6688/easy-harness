 ...
 * 见 `.codex/harness/spec/mechanical-gates.md` §8.5「审核加固项」。
 */
import {
...
test('R6 加强: docs/ 与 .codex/ 分支优先级不被扩展名规则改变（回归）', () => {
...
test('R29: 工具链授权凭证归类为 approval-marker（deny）', () => {
  const p = '.codex/toolchain-install-approved.json';
...
test('R29: 宪章与说明权威归类为 gate-config（改动等于调门禁口径）', () => {
  ...
  assert.equal(
    classifyHarnessSelfGovernedPath('.codex/harness/spec/mechanical-gates.md'),
    'gate-config',
  );
});
...
test('R29: harness-state.json 纳入角色门禁，期望 project-manager', () => {
  assert.equal(isHarnessStatePath('.harness/harness-state.json'), true);
  assert.equal(isGatedRoleArtifactPath('.harness/harness-state.json'), true);
  assert.deepEqual(expectedRolesForPath('.harness/harness-state.json'), ['project-manager']);
});
...
test('R28: 自治资产目标经 Shell 写入时按 R29 分级（而非普通路径判据）', () => {
  const forge = classifyShellWriteIntent(
    'Set-Content -Path .harness/.dispatched-roles.json -Value "{}"',
  );
...
test('R28: 只读 / 构建 / 框架运行器命令不被判为写文件（防误伤）', () => {
  for (const cmd of [
    ...
    'node .codex/scripts/e2e-run.mjs --scope=batch --required-ids=R-001',
    'node .codex/scripts/static-scan-run.mjs',
    'npm run lint',
    'npm test',
    'npx playwright test',
    'git status',
