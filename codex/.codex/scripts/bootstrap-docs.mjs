#!/usr/bin/env node
/**
 * 幂等初始化 docs/ 目录骨架 + process.md，并同步 `.harness/harness-state.json`。
 *
 * 职责：为门禁提供「活跃 process.md 指针」。Hook 通过 harness-state.json
 * （或环境变量 `HARNESS_PROCESS_PATH`）决定读哪一份流程文件。
 *
 * 用法：
 *   node .codex/scripts/bootstrap-docs.mjs                  # Greenfield → docs/
 *   node .codex/scripts/bootstrap-docs.mjs --feature=<name> # Feature → docs/<name>/
 *
...
const PROCESS_TEMPLATE = path.join(PROJECT_ROOT, '.codex/templates/process.md');
const HARNESS_STATE = path.join(PROJECT_ROOT, '.harness/harness-state.json');
...
/** 绝对路径 → workspace 相对正斜杠路径（写入 harness-state 用）。 */
...
/**
 * 更新活跃流程指针；保留文件中其它字段。
 * @param {string} activeProcessPath 相对 workspace 根的 process.md 路径
