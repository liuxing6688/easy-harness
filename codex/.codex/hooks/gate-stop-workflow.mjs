 ...
 *
 * 触发：`hooks.json` → `Stop`；Codex 没有 Cursor 的 `loop_limit`，适配器负责一次 continuation 防递归。
 ...
 *
 * 判据顺序的说明权威见 `.codex/harness/spec/mechanical-gates.md` §8.2（执行权威：Hook/脚本）。
 ...
 * 默认 fail-closed（注入 followup，不放行收尾），可由用户在 harness.config.json 改回 allow。
 ...
 *
 * stop 通道没有 `deny` 语义，「收紧」在这里等价于**不放行收尾**——注入 followup，
  ...
  try {
    ...
    const execProofFollowup = () =>
      exitFollowup(
        `【流程门禁】（R34 执行证明）以下门禁的机读产物未通过执行证明验签：${state.execProofFailedGates.join('、')}。` +
          ...
          '让 Bash PreToolUse 门禁签发 nonce 并由运行器落签；**禁止**手工编辑 test-results 下的机读产物。' +
          '若本机确实无法经门禁通道运行（如用户自行在外部终端执行），须由**用户本人**在 `.codex/harness.config.json` 设 `execProof.enforce: false`（R29 锁定，代理不得修改）。',
      );
    ...
    if (!isDocsOnly && state.qeComplete) {
      ...
      if (!state.lintPassed) {
        ...
        exitFollowup(
          '【流程门禁】（R15）QE 记录已完成，但编程规范（lint）门禁未通过。请由 quality-engineer 运行 `node .codex/scripts/lint-run.mjs` 并将违规整改至 gatePassed=true（机读产物 test-results/qe/.lint-result.json）；确无可用 linter 时须由 system-architect 在 gated-artifacts.json 声明 lintApplicability:"n/a" 且项目经理在 process.md「## 用户确认记录」补一行编程规范豁免确认。lint 未通过前不得推进测试或宣告完成。',
        );
      }
      // R16：静态代码质量硬门禁（重复代码 DRY + 安全静态扫描）。
      if (!state.staticScanPassed) {
        exitFollowup(
          '【流程门禁】（R16）QE 记录已完成，但静态代码质量门禁（重复代码 DRY + 安全静态扫描）未通过。请由 quality-engineer 运行 `node .codex/scripts/static-scan-run.mjs` 并将问题整改至 gatePassed=true（机读产物 test-results/qe/.static-scan-result.json）；确无法运行时须由 system-architect 在 gated-artifacts.json 分别声明 dupCheckApplicability/securityScanApplicability:"n/a" 且项目经理在 process.md「## 用户确认记录」补对应豁免确认。未通过前不得推进测试或宣告完成。',
        );
      }
      if (isHotfix) {
        ...
        if (state.finalTestRowComplete && !state.finalE2ePassed) {
          exitFollowup(
            '【流程门禁】（R11 hotfix 折叠通道）测试记录已完成，但 E2E 门禁未通过。请由 test-engineer 运行 `node .codex/scripts/e2e-run.mjs --scope=final --baseline=<requirement-list.md 或热修影响面>`；`gatePassed` 为 true 前禁止宣告完成。',
          );
        }
        ...
        if (!state.finalE2ePassed) {
          exitFollowup(
            '【流程门禁】（R37 single-task 折叠通道）测试记录已完成，但 E2E 门禁未通过。请由 test-engineer 运行 `node .codex/scripts/e2e-run.mjs --scope=final --baseline=<requirement-list.md 或本次增量范围>`；`gatePassed` 为 true 前禁止宣告完成。',
          );
        }
        ...
        if (state.batchTestRowComplete && !state.batchE2ePassed) {
          exitFollowup(
            '【流程门禁】本批次测试记录已完成，但批次 E2E 未通过。请由 test-engineer 运行 `node .codex/scripts/e2e-run.mjs --scope=batch --required-ids=<本批次P0>`；未通过前不得推进下一批次。',
          );
        }
        ...
        if (state.batchTestRowComplete && state.batchE2ePassed && !state.batchStorageReconPresent) {
          exitFollowup(
            '【流程门禁】（R17）本批次集成测试记录与批次 E2E 均已完成，但存储对账机读判据未满足。请由 test-engineer 在测试报告补全非空「## 存储对账记录」：须含适用分类型行（未豁免 R14 须含接口+非「不适用」介质行；未豁免 E2E 须含 E2E+非「不适用」介质行；至少一条真实对账适用行）；每行「关联任务包/对账方式/预期存储结果/实际存储结果/是否通过」非空；「存储介质」为数据库/文件/缓存/对象存储/其他/不适用（「其他」须备注具体系统；「不适用」仅用于无写入任务包留痕且须备注理由，不计入分类型真实对账）；且进度列表中已完成批次测试的任务包编号须全部出现在对账「关联任务包」列（见 `.codex/harness/spec/mechanical-gates.md` §8.3）。若本项目确无业务数据持久化，须由 system-architect 在 gated-artifacts.json 声明 storageReconciliationApplicability:"n/a" 且项目经理在 process.md「## 用户确认记录」补一行存储对账豁免确认，方可豁免本判据。',
          );
        }
        if (
        ...
        if (state.finalTestRequired) {
          if (state.finalTestRowComplete && !state.finalE2ePassed) {
            exitFollowup(
              '【流程门禁】最终测试记录已完成，但最终 E2E 未通过。请由 test-engineer 运行 `node .codex/scripts/e2e-run.mjs --scope=final --baseline=<requirement-list.md>`；未通过前禁止宣告完成。',
            );
          }
          if (state.finalTestRowComplete && state.finalE2ePassed && !state.startupSmokePassed) {
            exitFollowup(startupSmokeFollowup());
