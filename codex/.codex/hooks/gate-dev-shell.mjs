#!/usr/bin/env node
/**
 * Codex Bash PreToolUse 兼容内核（与 gate-toolchain-install 串联）。
 *
  ...
  try {
    ...
    if (!altStartup.ok) {
      deny(
        ...
        `AGENTS.md / test-engineer.toml：${altStartup.message ?? altStartup.reason}`,
      );
    }
    ...
    // 运行器命令正是走这条早退路径（`node .codex/scripts/*-run.mjs` 不匹配包管理正则）。
