#!/usr/bin/env node
/**
 * R13（需求 6）：成果物门禁链机械化 — Codex PreToolUse（Agent）。
 *
 * 职责：在角色 Task 真正发起前，对 `.codex/harness/spec/gate-chain.md` 表格中
 ...
 * @param {object} input Codex Hook stdin JSON
 * @returns {string|null}
 */
...
async function main() {
  ...
  try {
    ...
    // 供写入期角色↔路径匹配；不依赖 Codex 子代理 conversation_id 回链。
    if (role) {
      try {
