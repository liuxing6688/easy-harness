...
function hasAlternativeE2eStartupConfirmation(content) {
  ...
  for (const line of body.split('\n')) {
    ...
    if (!t.startsWith('|')) continue;
    if (/^\|[\s|:-]+\|?$/.test(t)) continue;
    // 须含「允许非 dist 启动做 E2E」语义（与 test-engineer.toml 强制约束对齐）
 ...
 * `node .codex/scripts/startup-smoke-run.mjs` 并取得 `gatePassed=true`。
 *
...
/** R17：具名存储介质关键词（与 `.codex/harness/spec/mechanical-gates.md` §8.3 一致；不含「其他」「不适用」） */
 ...
 * （`.codex/harness/spec/mechanical-gates.md` §8.3 说明权威；执行权威：Hook/脚本）。
...
export const LINT_FAILURE_GUIDANCE = Object.freeze({
  'no-lint-command':
    ...
    '①由**用户本人**在 `.codex/harness.config.json` 写 `qe.commands.lint` 覆盖（该文件受 R29 锁定，system-architect 与其它任何代理都不得代写，只能呈现片段请用户粘贴）；' +
  ...
  'lint-not-configured':
    ...
    '再由 quality-engineer 重跑 `node .codex/scripts/lint-run.mjs` 至 gatePassed=true。' +
