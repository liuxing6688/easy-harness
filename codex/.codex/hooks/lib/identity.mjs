...
export function recordIdentityBaselineNotice(reason) {
  try {
    ...
    const note = [
      ...
      `- [R5 身份判据降级] 本次受门禁写入在裁决时，顶层代写拦截处于 fail-open 降级态：${why}。此期间「顶层代理亲自写受门禁路径」**无法被机械拦截**，仅由 AGENTS.md §5.1 文字约束与回合自检兜底。若这不是预期状态，请检查 subagentStart Hook 是否生效、\`.harness/.root-conversation-id.json\` 是否被误写或被外部工具占用。`,
      '',
    ].join('\n');
 ...
 * Codex 当前 subagentStart/preToolUse 无法可靠把「子代理 conversation_id」映射回角色
