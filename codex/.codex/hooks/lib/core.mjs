...
// The adapter supplies the session workspace. The hook package location is
// only the fallback for direct legacy gate invocations and test runners.
export const PROJECT_ROOT = path.resolve(
  process.env.HARNESS_PROJECT_ROOT || path.resolve(HOOKS_DIR, '../..'),
);
export const RUNTIME_DIR = path.join(PROJECT_ROOT, '.harness');
export const DEFAULT_PROCESS_MD = path.join(PROJECT_ROOT, 'docs/process/process.md');
export const HARNESS_CONFIG = path.join(PROJECT_ROOT, '.codex/harness.config.json');
export const HARNESS_STATE = path.join(PROJECT_ROOT, '.harness/harness-state.json');
...
// R6：.codex/scripts|agents|hooks 三目录纳入机制门禁；其余 .codex/** 默认放行，
// 但可被 dotCodexExemptPatterns 精确豁免其中的非治理产物（如 .toolchain-install-approved.json）。
const DEFAULT_DOTCODEX_EXEMPT_PATTERNS = [
  '.codex/templates/**',
...
const DEFAULT_CONFIG = {
  gatedPaths: {
    ...
    dotCodexExemptPatterns: DEFAULT_DOTCODEX_EXEMPT_PATTERNS,
 ...
 * `{"permission":"allow"}`，属「Hook 成功」，故平台级失败策略不替代脚本自己的裁决
...
export function buildGateExceptionVerdict({ hook, context, err, channel, repairPaths = [] }) {
  ...
  const common =
    ...
    '由**用户本人**在 `.codex/harness.config.json` 设 `gateException.onJudgmentError: "allow"` ' +
    ...
    return {
      ...
      output: {
        followup_message:
          ...
          '确需恢复旧的 fail-open 行为时，须由**用户本人**在 `.codex/harness.config.json` 设 `gateException.onJudgmentError: "allow"`。',
      },
    };
  }
...
export function getMergedDotCodexExemptPatterns() {
  const config = loadHarnessConfig();
  return config.gatedPaths.dotCodexExemptPatterns ?? DEFAULT_DOTCURSOR_EXEMPT_PATTERNS;
}
