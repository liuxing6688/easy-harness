...
/**
 * Playwright E2E 配置（跨技术栈通用 · `.codex/harness/spec/mechanical-gates.md` §8.3 说明权威；执行权威：Hook/脚本）。
 * 仅支持 Chrome 内核浏览器（Chromium，涵盖 Chrome / Edge 等 Chromium-based 浏览器）；
 * JSON 报告供 `.codex/scripts/e2e-run.mjs` 按 projectName × status 解析。
 *
...
export default defineConfig({
  ...
  // 机读门禁产物固定写入 test-results/e2e/（见 .codex/scripts/e2e-run.mjs），须与此目录隔离。
