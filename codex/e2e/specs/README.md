...
## 编写约定（与 `test-engineer.toml` / `.codex/harness/spec/mechanical-gates.md` §8.3 保持一致）
...
3. **交互行为级断言**：断言须到具体交互结果（值/时机/落盘等），不得仅断言「页面加载成功」或「HTTP 200」。涉及业务数据写入时，存储对账机读与留痕见 **`.codex/harness/spec/mechanical-gates.md` §8.3 R17** / `checkBatchStorageReconciliationReport`（报告「## 存储对账记录」场景类型=E2E；介质含数据库/文件/缓存/对象存储等）。
...
5. 如需登记已解释的覆盖率豁免，创建 `e2e/coverage-waivers.json`（可选，格式见 `test-engineer.toml`）。
...
产物：`test-results/e2e/.e2e-batch-result.json` / `.e2e-final-result.json`（`gatePassed` 判据见 `.codex/harness/spec/mechanical-gates.md` §8.3，说明权威；执行权威：Hook/脚本）。
