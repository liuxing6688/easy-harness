...
1. 确认工作区根目录存在 `AGENTS.md` 与 `.codex/harness.config.json`（easy-harness 或已接入的宿主项目）。
2. 读取 `.harness/harness-state.json` 的 `activeProcessPath`；无则默认 `docs/process/process.md`。
3. 与用户确认复盘对象：
...
若 Phase 3 新增机械判定，在 `.codex/scripts/tests/selftest/` 或 `.codex/scripts/tests/scenarios/` 对应规则/场景文件中补最小回归用例（入口仍为 `gate-selftest.mjs` / `gate-scenarios.mjs`），并在变更清单中注明。
