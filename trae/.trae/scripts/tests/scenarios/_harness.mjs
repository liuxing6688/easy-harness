...
export function check(label, expect, opts) {
  ...
  const message = String(verdict.user_message ?? verdict.followup_message ?? verdict.reason ?? '');
...
export function writeLintNoCommand() {
  writeLintResultFixture({
    ...
    remediation: { configPath: '.trae/harness.config.json', suggestedCommand: null },
    executedAt: new Date().toISOString(),
  });
}
