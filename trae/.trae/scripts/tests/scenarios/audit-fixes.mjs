...
const CONFIRM_SINGLE_TASK = [
  ...
  '| 确认项 | 时间 | 用户原话摘要 |',
  '| ------ | ---- | ------------ |',
  '| 工作流模式确认 | 2026-07-30 | 确认采用 workflow_mode: single-task；AskUserQuestion「增量迭代」 |',
...
function evidencedBlockingProcess() {
  return [
    ...
    '| 确认项 | 时间 | 用户原话摘要 |',
    '| ------ | ---- | ------------ |',
    '| 阻塞决策 | 2026-07-30 | AskUserQuestion「等待支付沙箱开通」，用户选择暂停本批次 |',
    '',
    '## 进度列表',
    '',
    '| 角色/开发线 | 任务名称 | 状态 | 说明 |',
...
export function auditFixesScenarios() {
  ...
  check('AF4 R34/R29：写执行证明台账 .exec-proof-ledger.json 被拒', 'deny', {
    hook: 'write',
    filePath: '.trae/hooks/.exec-proof-ledger.json',
    processPath: r34Proc,
  });
  check('AF5 R34/R29：写私钥交接文件被拒（自签执行证明）', 'deny', {
    hook: 'write',
    filePath: '.trae/hooks/.exec-proof-pending/abc123.json',
    processPath: r34Proc,
  });
  check('AF6 R34/R29：Shell 删除私钥交接目录被拒（抹掉「未消费」作废条件）', 'deny', {
    hook: 'shell',
    command: 'rm -rf .trae/hooks/.exec-proof-pending',
    processPath: r34Proc,
  });
  check('AF7 R34：运行器命令本身仍放行（签发通道不得反过来拦住运行器）', 'allow', {
    hook: 'shell',
    command: 'node .trae/scripts/startup-smoke-run.mjs',
    processPath: r34Proc,
  });
