...
export function hardeningScenarios() {
  ...
  // R29：门禁自治资产
  // -------------------------------------------------------------------------
  check('SG1 R29：写 Hook 注册表 .trae/hooks.json 被拒（须用户本人改）', 'deny', {
    hook: 'write', filePath: '.trae/hooks.json', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG2 R29：写门禁配置 .trae/harness.config.json 被拒（须用户本人改）', 'deny', {
    hook: 'write', filePath: '.trae/harness.config.json', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG3 R29：伪造 R5 派发凭证 .dispatched-roles.json 被拒', 'deny', {
    hook: 'write', filePath: '.trae/hooks/.dispatched-roles.json', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG4 R29：改写 R5 身份基准 .root-conversation-id.json 被拒', 'deny', {
    hook: 'write', filePath: '.trae/hooks/.root-conversation-id.json', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG5 R29：自签工具链授权凭证被拒（须用户本人改）', 'deny', {
    hook: 'write', filePath: '.trae/hooks/.toolchain-install-approved.json', processPath: readyProc, gatedPath: readyGated,
  });
  ...
  check('SG7 R29：改写说明权威 mechanical-gates.md 被拒（须用户本人改）', 'deny', {
    hook: 'write', filePath: '.trae/harness/spec/mechanical-gates.md', processPath: readyProc, gatedPath: readyGated,
  });
  check('SG8 R29：harness-state.json 无活跃角色时允许 PM bootstrap', 'allow', {
    hook: 'write', filePath: '.trae/harness-state.json', processPath: noDispProc, gatedPath: noDispGated,
  });
  ...
  const shellCases = [
    ...
    ['SH8 框架运行器放行', 'node .trae/scripts/lint-run.mjs', 'allow'],
  ...
  check('SH12 Shell 伪造 R5 派发凭证被拒', 'deny', {
    hook: 'shell',
    command: 'Set-Content -Path .trae/hooks/.dispatched-roles.json -Value "{}"',
    processPath: noDispProc,
    gatedPath: noDispGated,
  });
  check('SH13 Shell 改写门禁配置被拒（须用户本人改）', 'deny', {
    hook: 'shell', command: 'echo {} > .trae/harness.config.json', processPath: noDispProc, gatedPath: noDispGated,
  });
