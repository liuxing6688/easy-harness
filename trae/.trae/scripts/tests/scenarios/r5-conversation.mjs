 ...
 * 覆盖 R5：顶层 session_id 代写拦截、角色↔路径、派发记录与身份基准。
 *
...
export function r5ConversationScenarios() {
  console.log('== R5 机械化补强：顶层会话 id 与调用者身份（session_id）==');
  ...
  check('C2 agent_id=solo_agent（顶层）时，即便分派计划有效仍拒绝写源码', 'deny', {
    hook: 'write', filePath: 'src/app.ts', processPath: readyProc, gatedPath: readyGated,
    agentId: 'solo_agent',
  });
  check('C3 agent_id 为子代理角色名时，有效分派计划写源码正常放行', 'allow', {
    hook: 'write', filePath: 'src/app.ts', processPath: readyProc, gatedPath: readyGated,
    agentId: 'development-engineer',
  });
  check('C4 payload 缺失 agent_id 字段时（fail-open）仍按既有分派计划判定放行', 'allow', {
    hook: 'write', filePath: 'src/app.ts', processPath: readyProc, gatedPath: readyGated,
  });
  check('C5 Shell 场景同理：agent_id=solo_agent 时拒绝受门禁初始化命令', 'deny', {
    hook: 'shell', command: 'npm install', processPath: readyProc, gatedPath: readyGated,
    agentId: 'solo_agent',
  });
  check('C6 Shell 场景：agent_id 为子代理角色名时按既有分派计划判定放行', 'allow', {
    hook: 'shell', command: 'npm install', processPath: readyProc, gatedPath: readyGated,
    agentId: 'development-engineer',
  });
