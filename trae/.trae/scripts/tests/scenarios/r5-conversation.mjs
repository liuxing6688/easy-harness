...
export function r5ConversationScenarios() {
  ...
  check('C2 session_id 等于顶层会话 id 时，即便分派计划有效仍拒绝写源码', 'deny', {
    hook: 'write', filePath: 'src/app.ts', processPath: readyProc, gatedPath: readyGated,
    conversationId: 'root-scn-abc',
  });
  check('C3 session_id 为子代理自己的 id（≠顶层）时，有效分派计划写源码正常放行', 'allow', {
    hook: 'write', filePath: 'src/app.ts', processPath: readyProc, gatedPath: readyGated,
    conversationId: 'subagent-scn-xyz',
  });
  check('C4 payload 缺失 session_id 字段时（旧行为兼容）仍按既有分派计划判定放行', 'allow', {
    hook: 'write', filePath: 'src/app.ts', processPath: readyProc, gatedPath: readyGated,
  });
  check('C5 Shell 场景同理：session_id 等于顶层会话 id 时拒绝受门禁初始化命令', 'deny', {
    hook: 'shell', command: 'npm install', processPath: readyProc, gatedPath: readyGated,
    conversationId: 'root-scn-abc',
  });
  check('C6 Shell 场景：session_id 为子代理 id 时按既有分派计划判定放行', 'allow', {
    hook: 'shell', command: 'npm install', processPath: readyProc, gatedPath: readyGated,
    conversationId: 'subagent-scn-xyz',
  });
