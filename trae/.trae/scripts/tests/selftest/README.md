...
| `r*.mjs` / `b1-*.mjs` | 对应规则用例 |
| `templates-vs-gates.mjs` | **出厂模板 ↔ 出厂门禁一致性**：加载 `.trae/templates/` 真实文件，断言被 Hook 解析的章节能定位、最小合规填充后判据真的通过 |
| `gated-artifacts-config.mjs` | R29 加强：`docs/**/design/gated-artifacts.json` 的角色门禁（仅 SA 可写，Write/RunCommand 同判） |
...
> （详见 `.trae/harness/spec/mechanical-gates.md` §8.5「解析层缺陷修复」）。
