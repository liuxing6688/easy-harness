...
import {
  ...
  checkIsomorphicModuleSectionReady, PROJECT_ROOT, extractPlannedRoles, checkDispatchPlanMatch,
} from './_harness.mjs';
...
console.log('== R39：分派计划匹配（R8 越级派发机械化）==');
test('R39: 无分派计划时 fail-open（PM 尚未写计划）', () => {
  fixtureProcess('---\nworkflow_mode: full\n---\n');
...
test('R39: 角色不在分派计划中时拒绝（PM 已计划其他角色）', () => {
  fixtureProcess(
    [
      '---',
      'workflow_mode: full',
...
test('R39: checkRoleDispatchGate 集成——角色不在分派计划时被拒', () => {
  fixtureProcess(
    [
      '---',
      'workflow_mode: full',
...
test('R39: extractPlannedRoles 解析两节合并去重', () => {
  const content = [
    '## 当前分派计划',
    '',
    '| 任务包编号 | 分派角色 | 并行/串行 | 状态 |',
