...
测试报告须标注每项场景的**测试类型**（集成 / E2E / 接口）。批次集成测试的测试报告须含非空「## 接口测试报告」章节（R14），并满足「## 存储对账记录」机读判据（R17，见 `.trae/harness/spec/mechanical-gates.md` §8.3）。
