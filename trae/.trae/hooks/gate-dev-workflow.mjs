 ...
 * 自锁防护（`.trae/harness/spec/mechanical-gates.md` §8.4）：workflow-gate-lib.mjs 动态加载失败或执行期出现未预期
...
async function main() {
  ...
  try {
    ...
    if (isRootConversationCaller(input?.session_id)) {
      deny(
        '流程门禁（R5，机械化补强）：检测到本次写入由顶层代理直接发起（session_id 与顶层会话一致），而非通过 Task 派发的子代理。受门禁路径必须由对应子 agent 在 Task 内执行。',
