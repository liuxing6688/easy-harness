 ...
 *
 * 消费方：`gate-dev-shell`（签发）、`.trae/scripts/*-run.mjs`（落签）、
 ...
 * ## 机制
 *
 * 1. **签发（Hook）**：`PreToolUse` 上 `gate-dev-shell` 识别出本次命令是框架
 *    自带运行器时，生成一对 ed25519 密钥：**公钥**写入台账 `.exec-proof-ledger.json`，
 *    **私钥**写入交接文件 `.exec-proof-pending/<nonce>.json`。两者都在 `.trae/hooks/`
...
export function detectRunnerExecProofKind(command) {
  ...
  const m = command.match(
    /\.trae[/\\]scripts[/\\](lint-run|static-scan-run|e2e-run|startup-smoke-run)\.mjs\b/i,
  );
  if (!m) return null;
 ...
 * 即 R15/R16/E2E/R32 这几项门禁真正在检查的东西。刻意**不含** `.trae/**`：
...
const REASON_MESSAGES = {
  ...
  'exec-proof-no-nonce':
    '产物自述未取到门禁签发的 nonce（execProof.nonce 为空）——本次运行器未经 PreToolUse 门禁签发，通常是在门禁之外的终端里执行的',
