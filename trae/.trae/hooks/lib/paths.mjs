...
import {
  ...
  loadHarnessConfig,
  getMergedGatedPaths,
  getMergeddotTraeExemptPatterns,
 ...
 */
export const GATE_REPAIR_TOOLS = Object.freeze(['write', 'edit', 'multiedit']);
...
export function isGatedDevPath(filePath) {
  ...
  // 注意：`.trae/`、`docs/`、`e2e/` 已在上方各自分支返回，不受本规则影响。
 ...
 * **R29**：门禁自治资产分类。
 *
 * 历史实现把 `.trae/hooks.json`、`.trae/harness.config.json` 与三个运行时标记
 * 全部放进 `dotTraeExemptPatterns`，等于「被约束方可以自由改写约束自身」——实测可
...
export const HARNESS_RUNTIME_MARKERS = Object.freeze([
  ...
  '.trae/hooks/.exec-proof-ledger.json',
  // **R35**：门禁异常事件旁路台账。可写等于可给自己伪造的「机器起源阻塞」补出处，
  // 从而用一行表格解除 stop 门禁的全部推进判据。
  '.trae/hooks/.gate-exception-ledger.json',
  // **R40**：闭环锁 marker。可写/可删等于可绕过「未闭环不得开始新开发」的跨回合约束——
  // 代理硬结束回合后下一轮 PreToolUse 据此收紧 DE 权限。只应由 stop hook 自身管理。
  '.trae/hooks/.workflow-closure-pending.json',
]);
 ...
 */
const EXEC_PROOF_PENDING_PREFIX = '.trae/hooks/.exec-proof-pending';
...
export function classifyHarnessSelfGovernedPath(filePath) {
  ...
  if (/^\.trae\/harness\/spec\/.+\.md$/.test(p)) return 'gate-config';
  return null;
}

/** `.trae/harness-state.json`：决定所有门禁读哪一份 process.md，归项目经理维护 */
export function isHarnessStatePath(filePath) {
  return normalizePath(filePath) === '.trae/harness-state.json';
}
...
const HARNESS_RUNNER_RE =
  /\.trae[\/\\]scripts[\/\\](?:e2e-run|lint-run|static-scan-run|startup-smoke-run|qe-run|bootstrap-docs|gate-selftest|gate-scenarios)\.mjs\b/i;
