 ...
 * （见 .trae/harness/spec/mechanical-gates.md §8.4；机械判定边界坦诚披露，非隐藏漏洞）。
 */
...
/**
 * R9 P0 报告结构化章节检测（P2-6 升级：软提醒留痕 + Stop 硬门禁阻断；说明权威见
 * `.trae/harness/spec/gate-chain.md` R9 脚注第 4 条（执行权威：Hook/脚本））：
 ...
 * **不做全目录关键词匹配**；缺失时返回 `needsReminder=true`，由 `gate-stop-workflow.mjs`
 * 两阶段处置：①`recordHotfixP0SoftReminder` 写一次性软性提醒到 `process.md` 留痕；
 * ②随后注入 followup **阻断收尾**（升级为 Stop 硬门禁）。本函数本身不写盘、不阻断，
 * 仅返回检测结果供调用方判定。
 */
