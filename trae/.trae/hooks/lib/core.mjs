 ...
 * 放行 verdict，属「Hook 成功」，故 `hooks.json` 里的 `failClosed: true`
 ...
 *   - `stop`：`block`（stop 通道没有 deny 语义，「收紧」即等于不放行收尾，对应 Trae Stop
 *     契约 `{decision:'block', reason}`）。
 *
 * Trae PreToolUse 输出契约（https://docs.trae.cn/ide_hook-configuration-reference）：
 *   { hookSpecificOutput: { hookEventName: 'PreToolUse', permissionDecision, permissionDecisionReason,
 *     additionalContext } }
 * Trae Stop 输出契约：`{ decision: 'block', reason }`。
 *
 * @param {{ hook: string, context: string, err: unknown, channel: 'write'|'shell'|'task'|'toolchain'|'stop', repairPaths?: string[] }} params
 * @returns {{ verdict: 'allow'|'deny'|'ask'|'block', output: object }}
 */
export function buildGateExceptionVerdict({ hook, context, err, channel, repairPaths = [] }) {
  ...
  const common =
    ...
    '由**用户本人**在 `.trae/harness.config.json` 设 `gateException.onJudgmentError: "allow"` ' +
    ...
    return {
      ...
      output: {
        ...
        reason:
          ...
          '确需恢复旧的 fail-open 行为时，须由**用户本人**在 `.trae/harness.config.json` 设 `gateException.onJudgmentError: "allow"`。',
      },
    };
  }
    ...
    return {
      ...
      output: {
        hookSpecificOutput: {
          hookEventName: 'PreToolUse',
          permissionDecision: 'ask',
          permissionDecisionReason: `工具链安装门禁（R36 判定期异常）：门禁在判定本次命令时抛出异常（${context}：${brief}），无法确定是否为需要授权的系统级安装命令，故转为请你确认。`,
          additionalContext: `${common} 若这确实是系统级工具链安装命令，请先按流程询问用户安装路径再重试。`,
        },
      },
    };
  }
  ...
  return {
    ...
    output: {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        permissionDecision: 'deny',
        permissionDecisionReason: `流程门禁（R36 判定期异常）：门禁在判定${subject}时抛出异常（${context}：${brief}），无法确定是否合规，故按 fail-closed 拒绝。（Hook：${hook}）`,
        additionalContext: `${common}${extra}`,
      },
    },
  };
}
 ...
 *
 * 机器起源分支原本只看 `process.md` 里有没有一行「待处理」，
...
export function hasBlockingDecisionTrace(content) {
  ...
  const stanceRe = /askuserquestion|用户|确认|决策|选择|答复|回复|裁决/i;
  for (const raw of body.split('\n')) {
    const line = raw.trim();
...
export function checkBlockingReleaseEvidence(content) {
  ...
  return {
    ...
    message:
      ...
      '（确认项含「阻塞」或「待决」，摘要含 AskUserQuestion 或用户决策内容）。' +
