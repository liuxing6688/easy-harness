  ...
  for (const dim of INCREMENT_SCOPE_DIMENSIONS) {
    ...
    if (dim.key === 'schema' && AFFECTED_YES_RE.test(row.affected)) {
      return {
        ...
        message:
          'R37：本次增量声明涉及数据模型 / schema 变更，**禁止**使用 single-task（`workflow-modes.md` 迭代分诊表既有规定，现补为机械判据）。schema 改动的兼容面与回滚面超出单轮折叠测试的覆盖能力，须经 AskUserQuestion 改回 `workflow_mode: full` 并走批次 + 最终两轮测试。',
      };
    }
  }
