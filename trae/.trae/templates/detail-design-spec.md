...
> 「是否受门禁保护」须与 `gated-artifacts.json`（见 §7）及 `.trae/harness.config.json` 默认列表核对一致。

## 4. 接口 / 数据模型设计

（按项目技术栈：API 契约、数据库 schema、状态管理等）

**业务数据存储介质（R17 输入，须明确）**：声明本项目业务数据是否写入持久化/缓存介质；若写入，列出类别（与 .trae/harness/spec/mechanical-gates.md §8.3 R17 存储介质范围一致：数据库 / 文件 / 缓存 / 对象存储 / 其他）。无上述写入时可走 R17 双要素豁免。写路径涉及几种介质，测试对账就覆盖几种。
...
> **机制说明**：QE 阶段运行 `node .trae/scripts/lint-run.mjs` 时，命令由 **`harness.config.json` → `qe.commands.lint` 覆盖 > 构建清单自动探测 > 下表默认值** 解析（与 Hook 机械判据一致）。本节由系统架构师按已确认技术栈**填入一行**，供 QE/PM 查阅；**不必**为通过门禁而重复写入 config——多数栈留空 `qe.commands.lint` 即可。
>
...
>
> **反弱化条款**：**禁止**以「减少打回」为由覆盖本命令调高阈值或扩大排除目录，详见 `.trae/harness/spec/mechanical-gates.md` §8.2 R16「反弱化条款」——确需覆盖须写明具体排除路径 + 理由，修改阈值须用户确认留痕。
...
已同步写入 `docs/[{feature}/]design/gated-artifacts.json`（模板：`.trae/templates/gated-artifacts.json`）。
