...
> **切勿**把外层 `easy-harness/` 当作工作区根——容器层没有 `.cursor/` / `.trae/` / `.codex/` / `AGENTS.md`，门禁与角色会**静默失效**。正确打开方式见下一节。

## 工作区根（重要）

仓库根 `easy-harness/` 只是容器（`.git` 在此），**本身不作为工作区根**。使用某一适配时，二选一：

- 直接把对应适配目录（如 `easy-harness/cursor/`、`easy-harness/trae/`、`easy-harness/codex/`）作为工作区根打开；或
...
各适配均已采用**薄宪章架构**：根 `AGENTS.md` 只常驻编排硬约束与权威索引；客观判据以 Hook/脚本为**执行权威**；长公式与细则在各自 `{.cursor|.trae|.codex}/harness/spec/`；角色操作细则在 `agents/*`（Task 时注入）。**根文件变薄 ≠ 规约变松。** 完整分层说明见各适配 README「规约权威分层」。

| 主题 | 要点 |
| ---- | ---- |
| **编排** | 7 角色分工；顶层不得代写（**R5**）或越级分派（**R8**）；PM 维护 `process.md`。Cursor/Trae 另有身份基准 TTL；Codex 以顶层 `read-only` + 角色 `workspace-write` 为主隔离 |
...
| **工具差异** | **Cursor**：Hook 自动拦截为主（含 `gate-subagent-track` 落盘顶层会话）。**Trae**：原生 Hook + `gate-check.mjs` 双保险，另有 `gate-r13-subagent`、**R39** 分派计划匹配、**R40** 闭环锁。**Codex**：`codex-hook-adapter` 做 wire-format 适配；顶层 `read-only` sandbox；`ask` 降级为 `deny`；运行时台账在 `.harness/`（因 `.codex/` / `.agents/` 原生只读）；Stop 允许一次 continuation |
...
1. 在仓库根**新增一个与 `cursor/` / `trae/` / `codex/` 平级的目录**（目录名取该工具名，如 `<tool>/`）。
