...
各适配均已采用**薄宪章架构**：根 `AGENTS.md` / `CLAUDE.md`（或该工具等价入口）只常驻编排硬约束与权威索引；客观判据以 Hook/脚本为**执行权威**；长公式与细则在各自 `{.cursor|.trae|.codex|.claude}/harness/spec/`；角色操作细则在 `agents/*`（分派时注入）。**根文件变薄 ≠ 规约变松。** 完整分层说明见各适配 README「规约权威分层」。
