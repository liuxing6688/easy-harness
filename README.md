...
2. **理解流程**：各适配 README 的框架全景 / 工作流模式（含 R20 轻量确认、R37 增量迭代档、§8.4–§8.8 硬化项；Trae §8.9 为 **R40** 闭环锁，Claude §8.9 为原生规则层）
...
各适配均已采用**薄宪章架构**：根 `AGENTS.md` / `CLAUDE.md`（或该工具等价入口）只常驻编排硬约束与权威索引；客观判据以 Hook/脚本为**执行权威**；长公式与细则在各自 `{.cursor|.trae|.codex|.claude}/harness/spec/`；角色操作细则在 `agents/*`（分派时注入）。Cursor / Trae / Claude 另有路径触发的**规则层**（只做提醒与转述，不新增约束；Claude 为官方 `.md` + `paths`，Cursor/Trae 为 `globs`）；Codex 无此层。**根文件变薄 ≠ 规约变松。** 完整分层说明见各适配 README「规约权威分层」。
...
2. 在该目录下实现一套**以自身为根、完整自洽**的适配。建议沿用薄宪章分层（不必与已有适配文件名一一对应，但应分清）：
   ...
   - **规则层（若该工具支持按路径注入提醒）**：如 `rules/`；只转述、不新增约束；硬禁令须同时留在宪章、角色文件或 Hook。扩展名与触发字段以该工具官方为准（Claude 为 `.md` + `paths`，勿照抄 Cursor 的 `.mdc` / `globs`）；
